...
# count vectorizer
def Postagging_count(doc_to_mat, min_df = 10, max_features = 100):
    vectorizer = TfidfVectorizer(tokenizer=mecab_tokenizer, min_df=min_df, max_features= max_features, ngram_range=(1, 1), stop_words=stopwords)
    cnt_to_vec = vectorizer.fit_transform(doc_to_mat)
    features = vectorizer.get_feature_names()
    nouns_df = pd.DataFrame(cnt_to_vec.toarray(), columns=features)

    vectorizer = TfidfVectorizer(tokenizer=mecab_tokenizer2, min_df=min_df, max_features= max_features, ngram_range=(1, 1), stop_words=stopwords)
    cnt_to_vec = vectorizer.fit_transform(doc_to_mat)
    features = vectorizer.get_feature_names()
    uqnouns_df = pd.DataFrame(cnt_to_vec.toarray(), columns=features)

    vectorizer = TfidfVectorizer(tokenizer=rhino_tokenizer1, min_df=min_df, max_features= max_features, ngram_range=(1, 1), stop_words=stopwords)
    cnt_to_vec = vectorizer.fit_transform(doc_to_mat)
    features = vectorizer.get_feature_names()
    adjec_df = pd.DataFrame(cnt_to_vec.toarray(), columns=features)

    vectorizer = TfidfVectorizer(tokenizer=komoran_tokenizer, min_df=min_df, max_features= max_features, ngram_range=(1, 1), stop_words=stopwords)
    cnt_to_vec = vectorizer.fit_transform(doc_to_mat)
    features = vectorizer.get_feature_names()
...
def save_pos_df(df):
    ...
    count_tdm.to_csv('../matrix/'+ str(input("save file name? : ")) + '_tfidf_tdm.csv', index= False)
    print('Matrix saved...')

if __name__ == "__main__":
    task_list = get_task_list(str(input("contents name? : ")))
    contents_df = import_data(task_list)
    df = preprocessing(contents_df)
    ...
    #corpus_df.to_csv('lang_review.csv', index = False)
    #print(pos_df.shape)
